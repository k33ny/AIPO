...
import MaastCTS.iw.NoveltyTester;
import MaastCTS.iw.StateMemory;
import MaastCTS.Agent;
import MaastCTS.Globals;
	...
	public void preSelect(StateObservation stateObs){
		...
		if(isFullyExpanded() && !noveltyTestedChildren){			
			if(mcts.allowsNoveltyBasedPruning()){
				...
				noveltyTester.setNoveltyTestMode(NoveltyTester.NOVELTY_TEST_MODES.One);
				...
				for(int i = 0; i < children.size(); ++i){
					...
					if(noveltyTester.noveltyTest(childStateMem, true) == NoveltyTester.NOVELTY_LEVELS.NotNovel){
						...
						while(parent != null){
							...
							if(parentNoveltyTester.noveltyTest(childStateMem, false) == NoveltyTester.NOVELTY_LEVELS.NotNovel){
