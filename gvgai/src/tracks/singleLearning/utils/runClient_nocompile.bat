...
if "%clientType%" == "java" (
    ...
    java -cp %build_folder% utils.JavaClient -agentName %path%  > logs/output_client_redirect.txt 2> logs/output_client_redirect_err.txt
	) 

if "%clientType%" == "python" (
    rem Run the PythonClient class
    c:\python35\python.exe %py_src_folder%\PythonClient.py %path%  > logs/output_client_redirect_python.txt 2> logs/output_client_redirect_err_python.txt
	)